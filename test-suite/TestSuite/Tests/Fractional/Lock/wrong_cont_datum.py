#!/usr/bin/python
"""
Test fractionalization with the wrong continuing datum.
"""
import os
import copy
...
def wrong_lock_cont_datum():
    """
    Build a fractionalization transaction that attemps to change the datum on the continuing output.
    """
    # env info
    ...
    next_script_datum['fields'][2]['bytes'] = "acab"
    p.write_json_file(next_script_datum, 'data/wrong_fractional_datum.json')
    ...
    utxo_out = [
        ...
        '--tx-out-inline-datum-file', 'data/wrong_fractional_datum.json',
        '--tx-out', newm_output,
    ]
