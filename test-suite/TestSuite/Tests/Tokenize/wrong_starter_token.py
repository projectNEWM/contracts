...
def wrong_starter_token():
    ...
    p.write_json_file(script_inline_datum[1], 'data/current_tokenized_datum.json')

    # build out the next datum
    next_script_datum = copy.deepcopy(script_inline_datum[1])
    ...
    tokenized_output = p.process_output(nft_lock_contract_addr, script_value[1])
