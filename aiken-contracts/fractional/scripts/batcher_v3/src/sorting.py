    ...
    for order1 in orders:
        for order2 in orders:
            ...
            if have1 == want2 and want1 == have2:
                ...
                p1 = Fraction(price1['fields'][0]['int'],
                              price1['fields'][1]['int'])
                s1 = Fraction(slip1['fields'][0]['int'],
                              slip1['fields'][1]['int'])

                price2 = order2[1]['datum']['fields'][2]['fields'][1]
                slip2 = order2[1]['datum']['fields'][2]['fields'][2]
                p2 = Fraction(price2['fields'][0]['int'],
                              price2['fields'][1]['int'])
                s2 = Fraction(slip2['fields'][0]['int'],
                              slip2['fields'][1]['int'])
                ...
                if low1 <= (1/p2) <= high1 and low2 <= (1/p1) <= high2:
                    ...
                    try:
                        amt1 = Fraction(
                            order1[1]['value'][have1['fields'][0]['bytes']][have1['fields'][1]['bytes']])
                        amt2 = Fraction(
                            order2[1]['value'][have2['fields'][0]['bytes']][have2['fields'][1]['bytes']])
                    except KeyError:
                        continue
                    ...
                    if getting1 >= want_thres1 and int(amt1) - getting2 >= have_thres1:
                        if getting2 >= want_thres2 and int(amt2) - getting1 >= have_thres2:
                            order_pairs = add_unique_tuple(
                                (order1[0], order2[0]), order_pairs)
          ...
          {'txid': '650941a13b743fbad71f67771772864d1b542e42bfc6b774dddf7bec36f43bfd#0',
           'datum': {
               'constructor': 0, 'fields': [
                   {'constructor': 0, 'fields': [
                       {'bytes': '2d5fec7bbb8abbe1fb6590db2676389dffab196d212fb2b4b9902dcc'}, {'bytes': ''}]},
                   {'constructor': 0, 'fields': [
                       {'bytes': '79b4b03d86b337231a3a861dba925675bf4edcce3169ad237feb0d14'}, {'bytes': '6c6f76656c616365'}]},
