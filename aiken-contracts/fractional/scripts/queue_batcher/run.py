...
def run():
    ...
    # create the address
    sale_address = address.create_script(sale_script_path, stake_script_path)
    queue_address = address.create_script(queue_script_path, stake_script_path)
    ...
    while True:
        ...
        order_book = orders.get_orders(
            order_book, sale_utxo_path, queue_utxo_path, pointer_pid_path)
        # print(order_book)
        ...
        for o in sorted_order_book:
            ...
            if queue_sc == "":
                ...
                buyer_address = Address(
                    payment_part=queue_vkh, network=Network.TESTNET).encode()
            else:
                print("\nBase Wallet")
                queue_kh = VerificationKeyHash(bytes.fromhex(queue_sc))
                buyer_address = Address(
                    payment_part=queue_vkh, staking_part=queue_kh, network=Network.TESTNET).encode()

            print("Buyer's Address Is", buyer_address)

            # the sale the queue item is pointing at
            queue_pointer_tkn = queue_data[queue_utxo]['inlineDatum']['fields'][4]['bytes']
            print(
                f"Buyer Wants To Purchase From {pointer_pid}.{queue_pointer_tkn}")

            # number of bundles wanted from queue item
            number_of_bundles = int(
                queue_data[queue_utxo]['inlineDatum']['fields'][2]['int'])

            print(f"Buyer Wants {number_of_bundles} Bundles")
            ...
            if found is False:
                print("\nA Queue Item Is Pointing At A Non-Existent Sale")
            ...
            if found_sale is False:
                print("\nCan't Find Sale By Pointer Token")
                continue
            ...
            try:
                ...
                if sale_value[bundle_pid][bundle_tkn] >= bundle_amt:
                    ...
                    if n_bundles >= number_of_bundles:
                        bundle_value = {bundle_pid: {
                            bundle_tkn: number_of_bundles * bundle_amt}}
            ...
            if value_exist_in_value(price_value, queue_value) is False:
                print("\nQueue Item Does Not Hold The Payment")
                continue
            ...
            # queue out value
            q_out_val = add_dicts(subtract_dicts(subtract_dicts(
                queue_value, incentive_value), price_value), bundle_value)
            q_out_val = subtract_dicts(q_out_val, {"lovelace": 600000})
            q_out_val = delete_zeros(q_out_val)

            # sale out value
            s_out_val = add_dicts(subtract_dicts(
            ...
            sign([batcher_skey, collat_skey],
                 network, '../tmp/tx-purchase.signed')

            print("Chaining Tx")
            id = txid('../tmp/tx-purchase.signed')
            next_sale_txid = id + "#1"
            ...
            print("Submit Tx")
            if DEBUG is False:
                submit(network, socket, '../tmp/tx-purchase.signed')
            if DEBUG is False:
                submit(network, socket, '../tmp/tx-refund.signed')
