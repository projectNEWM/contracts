...
if [[ $current_have_value -le 0 && $current_incentive_value -le 0 ]]; then
    script_address_out="${script_address} + ${lovelace_value}"
elif [[ $current_have_value -le 0 && $current_incentive_value -gt 0 ]]; then
    returning_asset="${current_incentive_value} ${ipid}.${itkn}"
    script_address_out="${script_address} + ${lovelace_value} + ${returning_asset}"
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file ../data/order_book/update-redeemer.json \
