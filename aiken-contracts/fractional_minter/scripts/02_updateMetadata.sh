...
updated_min_utxo=$(${cli} transaction calculate-min-required-utxo \
    --babbage-era \
    --protocol-params-file ./tmp/protocol.json \
    --tx-out-inline-datum-file ./data/updated-metadata-datum.json \
    --tx-out="${script_address} + 5000000 + ${asset}" | tr -dc '0-9')
...
if [ "$difference" -lt "0" ]; then
    ...
    variable=0; jq --argjson variable "$variable" '.fields[0].fields[0].int=$variable' ./data/update-redeemer.json > ./data/update-redeemer-new.json
    mv ./data/update-redeemer-new.json ./data/update-redeemer.json
else
    echo "Increase Min ADA by" ${difference}
    min_utxo=${updated_min_utxo}
    # update the increase ada in the redeemer
    variable=${difference}; jq --argjson variable "$variable" '.fields[0].fields[0].int=$variable' ./data/update-redeemer.json > ./data/update-redeemer-new.json
    mv ./data/update-redeemer-new.json ./data/update-redeemer.json
fi
...
cp ./data/updated-metadata-datum.json ./data/metadata-datum.json
