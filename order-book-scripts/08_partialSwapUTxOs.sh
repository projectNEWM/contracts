...
# update tx id info
variable=${array[0]}; jq --arg variable "$variable" '.fields[0].fields[0].bytes=$variable' data/redeemer/buyer_part_swap_redeemer.json > data/redeemer/buyer_part_swap_redeemer-new.json
mv data/redeemer/buyer_part_swap_redeemer-new.json data/redeemer/buyer_part_swap_redeemer.json
variable=${array[1]}; jq --argjson variable "$variable" '.fields[0].fields[1].int=$variable' data/redeemer/buyer_part_swap_redeemer.json > data/redeemer/buyer_part_swap_redeemer-new.json
mv data/redeemer/buyer_part_swap_redeemer-new.json data/redeemer/buyer_part_swap_redeemer.json
...
IFS='#' read -ra array <<< "$string"
# update tx id info
variable=${array[0]}; jq --arg variable "$variable" '.fields[0].fields[0].bytes=$variable' data/redeemer/seller_part_swap_redeemer.json > data/redeemer/seller_part_swap_redeemer-new.json
...
buyer_utxo_value=$(${cli} transaction calculate-min-required-utxo \
    --babbage-era \
    --protocol-params-file tmp/protocol.json \
    --tx-out="${script_address} + 5000000 + ${payment_asset}" | tr -dc '0-9')
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file data/redeemer/seller_part_swap_redeemer.json \
    --tx-in ${buyer_utxo} \
    --spending-tx-in-reference="${script_ref_utxo}#1" \
    --spending-plutus-script-v2 \
    --spending-reference-tx-in-inline-datum-present \
